    ...
    $(function() {
        ...
        $.get(("http://localhost:1337/user"), function(data) {
            ...
            $.each(data, function(index, data) {
                ...
                $("tbody").append(`<tr><td>${data.first_name}</td><td>${data.last_name}</td><td>${data.team}</td><td>${data.college}</td><td>${data.position}</td><td>${data.email}</td><td><button class="btnDelete" data="${data.id}">Delete</button></td>`);
